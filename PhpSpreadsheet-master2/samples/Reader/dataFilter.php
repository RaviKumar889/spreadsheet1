...
if(isset($_POST['columns'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        while($row = mysqli_fetch_assoc($result)) {                        
            $row_avg = "<tr role='row' class='odd' id='avgId'>
    ...
    if (mysqli_num_rows($count_result) > 0) {
        while($count_row = mysqli_fetch_assoc($count_result)) {                        
            $row_count = "<tr role='row' class='odd' id='countId'>
    ...
    if (mysqli_num_rows($max_result) > 0) {
        while($max_row = mysqli_fetch_assoc($max_result)) {                        
            $row_max = "<tr role='row' class='odd' id='maxId'>
    ...
    if (mysqli_num_rows($min_result) > 0) {
        while($min_row = mysqli_fetch_assoc($min_result)) {                        
            $row_min = "<tr role='row' class='odd' id='minId'>
...
if(isset($_POST['hiddenColumns']) && !empty($_POST['hiddenColumns']) ) {
    $finalArray =array();
    $arr2   =   explode(',',$_POST['average']);
    $arrayCal = array_diff($arr2 , $_POST['hiddenColumns']);
    $finalArray = array_merge($finalArray, $arrayCal);
    // print_r($arrayCal);die;
    //$fullDiff = array_merge(array_diff( $_POST['hiddenColumns'],$arr2), array_diff($arr2, $_POST['hiddenColumns']));

    for( $i=1;$i<count($finalArray) ; $i++) {
        if($finalArray[$i] == 'Ra_id') {
            $finalArray[$i] = "ra_id";
        }

        if($finalArray[$i] == 'Campaign_name') {            
            $finalArray[$i] = "campaign_name";
        }
        
        if($finalArray[$i] == 'Company_email') {
            $finalArray[$i] = "company_email";
        }
        
        if($finalArray[$i] == 'Category') {
            $finalArray[$i] = "category";
        }
        
        if($finalArray[$i] == 'Sub_category') {
            $finalArray[$i] = "sub_category";
        }
        
        if($finalArray[$i] == 'Content_Length') {
            $finalArray[$i] = "content_length";
        }
        
        if($finalArray[$i] == 'Content_url') {
            $finalArray[$i] = "content_url";
        }
        ...
        if($finalArray[$i] == 'Status') {
            $finalArray[$i] = "status";
        }

        if($finalArray[$i] == 'Lift') {
            $finalArray[$i] = "lift";
        }
        

        if($finalArray[$i] == 'urgency') {
            $finalArray[$i] = "urgency";
        }
        
        if($finalArray[$i] == 'Intent') {
            $finalArray[$i] = "Intent";
        }
        
        if($finalArray[$i] == 'Reaction_Ratio') {
            $finalArray[$i] = "reaction_ratio";
        }
        
        if($finalArray[$i] == 'Share') {
            $finalArray[$i] = "share";
        }
        
        if($finalArray[$i] == 'Recall') {
            $finalArray[$i] = "recall";
        }
        
        if($finalArray[$i] == 'Trust') {
            $finalArray[$i] = "trust";
        }
        
        if($finalArray[$i] == 'Virality') {
            $finalArray[$i] = "virality";
        }
        
        if($finalArray[$i] == 'Relatable') {
            $finalArray[$i] = "relatable";
        }
        
        if($finalArray[$i] == 'Relevance') {
            $finalArray[$i] = "relevance";
        }
        
        if($finalArray[$i] == 'attention') {
            $finalArray[$i] = "attention";
        }
        
        if($finalArray[$i] == 'Emotion_End') {
            $finalArray[$i] = "emotion_end";
        }
        
        if($finalArray[$i] == 'Awareness') {
            $finalArray[$i] = "awareness";
        }
        
        if($finalArray[$i] == 'Brand') {
            $finalArray[$i] = "brand";
        }
        
        if($finalArray[$i] == 'headroom_lift') {
            $finalArray[$i] = "headroom_lift";
        }
        
        if($finalArray[$i] == 'Positive_attitude') {
            $finalArray[$i] = "positive_attitude";
        }
        
        if($finalArray[$i] == 'Negative_attitude') {
            $finalArray[$i] = "negative_attitude";
        }
        
        if($finalArray[$i] == 'pre_interest') {
            $finalArray[$i] = "pre_interest";
        }
        
        if($finalArray[$i] == 'post_interest') {
            $finalArray[$i] = "post_interest";
        }
        
        if($finalArray[$i] == 'Urgency_Headroom_lift') {
            $finalArray[$i] = "urgency_headroom_lift";
        }
        
        if($finalArray[$i] == 'Positive_Urgency') {
            $finalArray[$i] = "positive_urgency";
        }
        
        if($finalArray[$i] == 'Negative_Urgency') {
            $finalArray[$i] = "negative_urgency";
        }
        
        if($finalArray[$i] == 'Urgency_Pre_Interest') {
            $finalArray[$i] = "urgency_pre_interest";
        }
        
        if($finalArray[$i] == 'Urgency_Post_Interest') {
            $finalArray[$i] = "urgency_post_interest";
        }
        
        if($finalArray[$i] == 'Subscriber_intent') {
            $finalArray[$i] = "subscriber_intent";
        }
        
        if($finalArray[$i] == 'Persuasion') {
            $finalArray[$i] = "persuasion";
        }
        
        if($finalArray[$i] == 'continuity') {
            $finalArray[$i] = "continuity";
        }
        
        if($finalArray[$i] == 'Enjoyment') {
            $finalArray[$i] = "enjoyment";
        }
        
        if($finalArray[$i] == 'Valence') {
            $finalArray[$i] = "valence";
        }
        
        if($finalArray[$i] == 'Arousal') {
            $finalArray[$i] = "arousal";
        }
        
        if($finalArray[$i] == 'Reaction_intensity') {
            $finalArray[$i] = "reaction_intensity";
        }
        
        if($finalArray[$i] == 'Attention_New') {
            $finalArray[$i] = "attention_new";
        }
        
        if($finalArray[$i] == 'Emotional_Engagement') {
            $finalArray[$i] = "emotional_engagement";
        }

        if($finalArray[$i] == 'Action') {
            $finalArray[$i] = "action";
        }
    }

    $c = 0; $col=""; $table_row = "";$columnsLast=array();
    ...
    if (mysqli_num_rows($result) > 0) {
        $default ='<tr id="loadAvgId"><td>Avg</td>';   
        $a='';     
        while($row = mysqli_fetch_assoc($result)) {            
